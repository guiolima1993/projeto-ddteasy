...
let plugin = {
    ...
    beforeDraw: (chart) => {
        ...
        ctx.fillStyle = 'rgba(88, 37, 131, 0.25)';
    ...
    if (cep != "") {
        ...
        if (validacep.test(cep)) {
            $.getJSON("//viacep.com.br/ws/" + cep + "/json/", function(dados) {
                if (!("erro" in dados)) {
                    $(_parent).find(".fill-endereco").html(dados.logradouro + ", " + dados.bairro + ", " + dados.localidade + ", " + dados.uf ).blur();
                    $(_parent).find(".fill-endereco").val(dados.logradouro + ", " + dados.bairro + ", " + dados.localidade + ", " + dados.uf ).blur();
                    
                    $('#search-job').attr('disabled', false);
                } else {
                    clear_cep(_parent);
...
/**
 * Função que verifica se há cookies no navegador
 */
...
$(function() {
    ...
    if ($('.main-banner').length) {
        ...
        mainBanner.owlCarousel({
            ...
            nav: true,
            navText: [
                '<img src=\"' + base + 'images/ddteasy-images/chevron-back-outline.svg\">',
                '<img src=\"' + base + 'images/ddteasy-images/chevron-forward-outline.svg\">'
            ]
        });
    ...
    if ($('form').length) {
        ...
        $('.form-submit').submit(function(e) {
            ...
            if(!valid){
                ...
                if(_this.attr("id") == "form-cadastrar"){
                    let testPwd = document.getElementById("pwd-place");
                    let confirmarPwd = document.getElementById("pwd-repeat");                    
                    let pwdValUppercase = /[A-Z]/g;
                    let pwdValNumber = /[0-9]/g;
                    let pwdValSpecial = /[@#$]/g

                    if(testPwd.value.match(pwdValUppercase) && testPwd.value.match(pwdValNumber) && testPwd.value.match(pwdValSpecial) && testPwd.value.length >= 8) {
                        if(testPwd.value == confirmarPwd.value){
                            if (recaptcha === "") {
                                e.preventDefault();
                                ...
                                $.ajax({
                                    ...
                                    success: function(data){
                                        if (data == 1) {
                                            if (_this.attr('id') === "form-parceiro"){
                                                swal({
                                                    title: "Sucesso",
                                                    text: "Sua mensagem foi enviada com sucesso. Agradecemos o interesse e em breve entraremos em contato.",
                                                    icon: "success",
                                            ...
                                            }else if (_this.attr('id') == "form-cadastrar"){
                                                swal({
                                                    title: "Sucesso",
                                                    text: "Seu cadastro foi realizado com sucesso! Você será redirecionado(a) para a página de login.",
                                                    icon: "success",
                                                    confirmButtonText: "Ok"
                                                }).then(function(confirm){
                                                    if(confirm){
                            ...
                            swal("Erro", "A confirmação da sua senha deve ser idêntica a senha escolhida.", "info");
                        }      

                    }else {
                        swal("Erro", "A sua senha deve conter pelo menos 8 dígitos, uma letra maiúscula, um número e um caráctere especial.", "info");
                    }
